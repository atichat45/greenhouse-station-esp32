...
#define WIFI_SSID "IP-0349" //OMAKASE_2.4G
#define WIFI_PASSWORD "yoyoyoyo" //0936540643
...
//control Topic
#define MQTT_PUB_L1 "Light1"
#define MQTT_PUB_L2 "Light2"
#define MQTT_PUB_L3 "Light3"
#define MQTT_PUB_L4 "Light4"
#define MQTT_PUB_P1 "Pump1"
#define MQTT_PUB_P2 "Pump2"
#define MQTT_PUB_S1 "Servo1"
...
void onMqttConnect(bool sessionPresent) {
  ...
  int16_t packetIdSub_L1 = mqttClient.subscribe(MQTT_SUB_L1, 0);
  int16_t packetIdSub_L2 = mqttClient.subscribe(MQTT_SUB_L2, 0);
  int16_t packetIdSub_L3 = mqttClient.subscribe(MQTT_SUB_L3, 0);
  int16_t packetIdSub_L4 = mqttClient.subscribe(MQTT_SUB_L4, 0);
  int16_t packetIdSub_P1 = mqttClient.subscribe(MQTT_SUB_P1, 0);
  int16_t packetIdSub_P2 = mqttClient.subscribe(MQTT_SUB_P2, 0);
  int16_t packetIdSub_S1 = mqttClient.subscribe(MQTT_SUB_S1, 0);
...
void onMqttMessage(char* topic, char* payload, AsyncMqttClientMessageProperties properties, size_t len, size_t index, size_t total) {
  ...
  //   Serial.println("TRUE");
  //  }
  if (strcmp(topic, MQTT_SUB_L1) == 0) {
     Serial.println("L1 Publish received.");
     //Check and turn on/off Light1
     if(strcmp(messageTemp.c_str(), "on") == 0){
      digitalWrite(L1Pin, HIGH);
      Serial.println("Status: L1 on");
     }
     else if(strcmp(messageTemp.c_str(), "off") == 0){
      digitalWrite(L1Pin, LOW);
      Serial.println("Status: L1 off");
     }
    }
  if (strcmp(topic, MQTT_SUB_L2) == 0) {
    Serial.println("L2 Publish received.");
    //Check and turn on/off Light2
    if(strcmp(messageTemp.c_str(), "on") == 0){
     digitalWrite(L2Pin, HIGH);
     Serial.println("Status: L2 on");
    }
    else if(strcmp(messageTemp.c_str(), "off") == 0){
     digitalWrite(L2Pin, LOW);
     Serial.println("Status: L2 off");
    }
  }
  if (strcmp(topic, MQTT_SUB_L3) == 0) {
    Serial.println("Status: L3 Publish received.");
    //Check and turn on/off Light4
    if(strcmp(messageTemp.c_str(), "on") == 0){
     digitalWrite(L3Pin, HIGH);
     Serial.println("Status: L3 on");
    }
    else if(strcmp(messageTemp.c_str(), "off") == 0){
     digitalWrite(L3Pin, LOW);
     Serial.println("Status: L3 off");
    }
  }
  if (strcmp(topic, MQTT_SUB_L4) == 0) {
    Serial.println("L4 Publish received.");
    //Check and turn on/off Light4
    if(strcmp(messageTemp.c_str(), "on") == 0){
     digitalWrite(L4Pin, HIGH);
     Serial.println("Status: L4 on");
    }
    else if(strcmp(messageTemp.c_str(), "off") == 0){
     digitalWrite(L1Pin, LOW);
     Serial.println("Status: L4 off");
    }
  }
  if (strcmp(topic, MQTT_SUB_P1) == 0) {
    Serial.println("P1 Publish received.");
    //Check and turn on/off Pump1
    if(strcmp(messageTemp.c_str(), "on") == 0){
     digitalWrite(P1Pin, HIGH);
     Serial.println("Status: P1 on");
    }
    else if(strcmp(messageTemp.c_str(), "off") == 0){
     digitalWrite(P1Pin, LOW);
     Serial.println("Status: P1 off");
    }
  }
  if (strcmp(topic, MQTT_SUB_P2) == 0) {
    Serial.println("P2 Publish received.");
    //Check and turn on/off Fan
    if(strcmp(messageTemp.c_str(), "on") == 0){
  ...
  if (strcmp(topic, MQTT_SUB_S1) == 0) {
    Serial.println("S1 Publish received.");
    //Check and turn on/off Servo
    if(strcmp(messageTemp.c_str(), "on") == 0){
     digitalWrite(S1Pin, HIGH);
     Serial.println("Status: Servo1 on");
    }
    else if(strcmp(messageTemp.c_str(), "off") == 0){
     digitalWrite(S1Pin, LOW);
     Serial.println("Status: Servo1 off");
    }
  }
...
void loop() {
  ...
  //Check condition of temp on Fan
  if(t>=35){
    digitalWrite(P2Pin, HIGH);
    //uint16_t packetIdPub6 = mqttClient.publish(MQTT_PUB_P2, 1, true, "on");
  }
  else{
    digitalWrite(P2Pin, LOW);
    //uint16_t packetIdPub6 = mqttClient.publish(MQTT_PUB_P2, 1, true, "off");
  }
